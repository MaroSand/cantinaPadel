...
//  Crédito: plata que el cliente ya entregó pero que no alcanzó para saldar por completo la próxima unidad (se guarda en clientes.saldo_cuenta_corriente)
//  Deuda neta: lo que efectivamente falta cobrar (deuda bruta - crédito)
//  Saldo a favor: solo existe si el crédito supera la deuda bruta, es decir, cuando el cliente pagó de más
public static class CalculadorCuentaCorriente
{
    // Aplica el crédito a las unidades pendientes en orden FIFO
    // Una unidad solo se salda si el crédito la cubre por completo, en cuanto una no alcanza se corta para que el sobrante siempre se acumule
    // sobre la unidad más antigua que sigue impaga
